...
<script>
	...
	const toolbarOptions = [
		...
		["clean"], // 清除文本格式
		["link", "image", "video", "audio", "continuation", "optimize", "proofread", "translation"], // 链接、图片、视频
	];
	var _EditorOption_ = function(page_this) {
		return {
			modules: {
				toolbar: {
					...
					handlers: {
						...
						continuation(value) {
							page_this.handleAIContinuation()
						},
						optimize(value) {
							page_this.handleAIOptimize()
						},
						proofread(value) {
							page_this.handleAIProofread()
						},
						translation(value) {
							page_this.handleAITranslation()
						}
					},
				},
			},
			...
			initButton: function () {
				...
				const continuation= document.querySelector(".ql-continuation");
				continuation.title = 'AI续写'
				continuation.style.cssText =
					"width:28px;height: 24px;color: #000";
				continuation.className = "iconfont icon-gerenzhongxin-zhihui";
				
				const optimize= document.querySelector(".ql-optimize");
				optimize.title = 'AI优化'
				optimize.style.cssText =
					"width:28px;height: 24px;color: #000";
				optimize.className = "iconfont icon-renlian01";
				
				const proofread= document.querySelector(".ql-proofread");
				proofread.title = 'AI校对'
				proofread.style.cssText =
					"width:28px;height: 24px;color: #000";
				proofread.className = "iconfont icon-touxiang03";
				
				const translation= document.querySelector(".ql-translation");
				translation.title = 'AI翻译'
				translation.style.cssText =
					"width:28px;height: 24px;color: #000";
				translation.className = "iconfont icon-fanhui01";
			},
		};
	};
	...
	export default {
		...
		data() {
			return {
				...
				content: this.value,
				quillUpdateImg: false, // 根据图片上传Status来ConfirmedSURE显示loading动画，刚开始是false,不显示
		...
		methods: {
			...
			handleAIContinuation() {
				...
				var div = document.createElement('div')
				div.innerHTML = this.value
				var text = div.textContent + ' 请将前面的Content续写下去'
			...
			handleAIOptimize() {
				...
				let that = this
				if(range.length<=range.index) {
					this.$message.error('请选中需要优化的Content')
					return false
				}
				let loading = this.$loading({
					lock: true,
				...
				var div = document.createElement('div')
				div.innerHTML = this.value
				var text = quill.getText(range.index, range.length) + ' 请帮我优化一下这段文字的Content，并只Back优化的Content'
				this.$http.post('deepseek/askai', {
					ask: `${text}`,
				}).then(res => {
					if (loading) loading.close()
			...
			handleAIProofread() {
				...
				let that = this
				if(range.length<=range.index) {
					this.$message.error('请选中需要校对的Content')
					return false
				}
				let loading = this.$loading({
					lock: true,
				...
				var div = document.createElement('div')
				div.innerHTML = this.value
				var text = quill.getText(range.index, range.length) + ' 请帮我校对一下这段文字的Content，并只Back校对的Content'
				this.$http.post('deepseek/askai', {
					ask: `${text}`,
				}).then(res => {
					if (loading) loading.close()
			...
			handleAITranslation() {
				...
				let that = this
				if(range.length<=range.index) {
					this.$message.error('请选中需要翻译的Content')
					return false
				}
				let loading = this.$loading({
					lock: true,
				...
				var div = document.createElement('div')
				div.innerHTML = this.value
				var text = quill.getText(range.index, range.length) + ' 请帮我翻译一下这段文字的Content，并只Back翻译Content'
				this.$http.post('deepseek/askai', {
					ask: `${text}`,
				}).then(res => {
					if (loading) loading.close()
