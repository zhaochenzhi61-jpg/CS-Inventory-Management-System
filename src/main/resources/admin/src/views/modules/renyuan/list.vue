		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #fff","width":"100%","padding":"0","margin":"0","borderRadius":"16px","background":"#C2D7F2"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"30px","position":"relative","flexWrap":"wrap","display":"flex"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">Account</label>
						<el-input v-model="searchForm.zhanghao" placeholder="Account" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select" label="Type" prop="leixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"400","height":"40px"}' class="item-label">Type</label>
						<el-select clearable v-model="searchForm.leixing" placeholder="Please select Type" >
				...
				<el-row class="actions" :style='{"width":"100%","padding":"0 30px 30px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('renyuan','Add')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						Add
					</el-button>
					<el-button class="del" v-if="isAuth('renyuan','Delete')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #fff","width":"100%","padding":"20px","margin":"30px 0 0 0","borderRadius":"16px","background":"#C2D7F2"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="touxiang" width="200" label="Profile picture">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="Operation">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('renyuan','View')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								View
							</el-button>
							<el-button class="edit" v-if=" isAuth('renyuan','Modify') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('renyuan','Delete')" type="primary" @click="deleteHandler(scope.row.id)">
...
<script>
	...
	export default {
		...
		methods: {
			...
			init () {
				this.leixingOptions = "Student,Teacher".split(',')
			},
			search() {
				this.pageIndex = 1;
				this.getDataList();
