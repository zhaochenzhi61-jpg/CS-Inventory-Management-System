		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #fff","width":"100%","padding":"0","margin":"0","borderRadius":"16px","background":"#C2D7F2"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"100%","padding":"0 30px 30px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('config/' + configStatus,'Add')&&configStatus==1" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						Add
					</el-button>
					<el-button class="del" v-if="isAuth('config/' + configStatus,'Delete')&&configStatus==1" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #fff","width":"100%","padding":"20px","margin":"30px 0 0 0","borderRadius":"16px","background":"#C2D7F2"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('config/' + configStatus,'View')"
					:data="dataList"
					...
					<el-table-column width="300" label="Operation">
						<template slot-scope="scope">
							<el-button class="view" v-if="isAuth('config/' + configStatus,'View')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								View
							</el-button>
							<el-button class="edit" v-if="isAuth('config/' + configStatus,'Modify')" type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('config/' + configStatus,'Delete')&&configStatus==1" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
...
<script>
	...
	export default {
		...
		watch: {
			'$route' (to, from) { //监听路由SURE变化
				if(this.$route.params.type){//判断StatusSURE有值
				//调数据
