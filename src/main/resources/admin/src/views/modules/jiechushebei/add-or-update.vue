	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="Borrownumber" prop="keyongshuliang" >
					<el-input v-model.number="ruleForm.keyongshuliang" placeholder="Borrownumber" clearable  :readonly="ro.keyongshuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="Borrownumber" prop="keyongshuliang" >
					<el-input v-model="ruleForm.keyongshuliang" placeholder="Borrownumber" readonly></el-input>
				</el-form-item>
				<el-form-item class="date" v-if="type!='info'" label="Loantime" prop="jieyongshijian" >
					<el-date-picker
						...
						placeholder="Loantime"
					></el-date-picker> 
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.jieyongshijian" label="Loantime" prop="jieyongshijian" >
					<el-input v-model="ruleForm.jieyongshijian" placeholder="Loantime" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="select" v-if="type!='info'"  label="Status" prop="zhuangtai" >
					<el-select :disabled="ro.zhuangtai" v-model="ruleForm.zhuangtai" placeholder="Please select Status"  filterable>
						<el-option
				...
				<el-form-item v-else class="input" label="Status" prop="zhuangtai" >
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.jieyongshijian = this.getCurDate()
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.jiechushuliang = 0
					this.ro.jiechushuliang = false
					this.ruleForm.keyongshuliang = 0
					this.ro.keyongshuliang = false
					this.ruleForm.zhuangtai = 'Unreturned'; 				}
				// 获取用户信息
				this.$http({
					...
					if (data && data.code === 0) {
						...
						if(((json.leixing!=''&&json.leixing) || json.leixing==0) && this.sessionTable!="users"){
							this.ruleForm.leixing = json.leixing
							this.ro.leixing = true;
						}
				...
				this.zhuangtaiOptions = "Returned,Unreturned".split(',')
			...
			async onSubmit() {
					...
					if(objcross!=null) {
						if(!this.ruleForm.jiechushuliang){
							this.$message.error("Loan-Num Cannot be empty");
							return
						}
					}
					var table = this.$storage.getObj('crossTable');
					if(objcross!=null) {
						if(!this.ruleForm.keyongshuliang){
							this.$message.error("Borrownumber Cannot be empty");
							return
						}
						if(objcross.keyongshuliang<this.ruleForm.keyongshuliang){
							this.$message.error("Borrownumber不足");
							return
						}
					}
					if(!this.ruleForm.id) {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiechushebei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiechushebeiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
